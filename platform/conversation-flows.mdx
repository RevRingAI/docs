...
| `extension` | string | No | Touch-tone digits dialed after the call connects, to reach an extension behind a phone menu. Allowed: `0-9`, `*`, `#`, and `w` for a 0.5-second pause. Start with `wwww` (2 seconds) so the destination is ready for input, e.g. `"wwww1234"`. Warm transfers only. |
