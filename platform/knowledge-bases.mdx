...
**Dashboard:** Click **Add Source** → **Text**, enter a name and content, then click **Add**.
...
**Dashboard:** Click **Add Source** → **URL**, enter the URL and an optional display name, then click **Add**.
...
Upload documents directly. Supported formats include PDF, TXT, DOCX, CSV, and other common document types.
...
The response includes a `status` field (`processing`, `ready`, or `failed`) and an `errorMessage` if processing failed.
...
4. Select the knowledge base to link
5. Click **Save**
...
| Mode | Description |
|------|-------------|
| `automatic` | The agent automatically searches the knowledge base whenever it might be helpful. This is the default and recommended mode. |
| `query_tool` | The agent uses a dedicated tool to search the knowledge base. This gives you more control over when retrieval happens — useful if you want the agent to only search when explicitly asked. |
...
Minimum relevance score for retrieved passages (0–1, default 0). Set this higher (e.g., 0.3–0.5) to filter out loosely related content. A value of 0 returns the top results regardless of relevance score.
...
**Dashboard:** Click the **Delete** button next to the source.
...
**Dashboard:** Open the knowledge base and click **Delete**.
