...
In voice tests, transfers are simulated the same way: the test call is isolated by design, so instead of dialing a live phone line the transfer completes in simulation and rubrics score the agent's transfer behavior normally. To test how your agent handles a failed transfer (for example a take-a-message flow), set the variable `_sim_transfer` to `fail` on the test case; the simulated transfer then fails and the agent follows its failure instructions. The same variable works in chat tests.
