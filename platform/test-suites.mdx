...
In voice tests, a transfer cannot actually connect to a live phone line (the test call is isolated by design), so the transfer attempt fails and the agent continues per its failure instructions. For test cases that assert transfer behavior, use chat mode; use voice runs for audio, turn-taking, and greeting checks.
