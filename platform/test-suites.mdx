...
5. Choose the **Test Type**: **Chat** (default) for fast text conversations, or **Voice** for full simulated calls
...
When starting a run you can override the suite's test type for that run only, for example a final voice validation on a chat suite: pass `{ "mode": "voice" }` to the run endpoint.
