...
- **Tester agent** — an optional second agent that plays the role of the caller (if omitted, RevRing uses a built-in tester)
...
5. Optionally select a **Tester Agent** (a separate agent that role-plays as the caller)
6. Optionally set **Max Concurrency** to control how many test calls run in parallel (default 2, max 100)
7. Optionally set **Default Variables** to pass to the target agent for all test calls
8. Click **Create**
...
**Dashboard:** Open your test suite, click **Add Test Case**, fill in the name, script, and scoring rubric.
...
**Dashboard:** Click **Generate Tests** in your test suite, enter a prompt, and review the generated cases.
...
**Dashboard:** Click **Run Tests** in your test suite. The run starts immediately and you can monitor progress in real time.
...
**Dashboard:** The test run detail page shows real-time progress with results for each attempt.
...
**Dashboard:** Click **Cancel** on a running test run.
...
The `maxConcurrency` setting controls how many test calls run simultaneously. Higher concurrency completes runs faster but counts against your organization's call concurrency limit. Start with a low value (2–5) and increase as needed.
...
<AccordionGroup>
  ...
  <Accordion title="Tester agent not following the script">
    If using a custom tester agent, ensure its prompt instructs it to follow the test script closely. The script is injected into the tester agent's context, but a conflicting system prompt may override it.
  </Accordion>
</AccordionGroup>
