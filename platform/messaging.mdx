...
Connect a messaging provider so your agents can send text messages. Today your agents can send an SMS **during a live call** (for example, texting the caller a booking or payment link), using the same number they call from. Support for more messaging workflows is on the way.
