...
The [Send Call](https://dashboard.revring.ai/app/testing/send) page is the primary way to test outbound calls from the dashboard.
...
### When the Phone Is Still Ringing

Some carrier routes report a call as answered before the person has actually picked up, occasionally by 15 seconds or more. Anything the agent says in that window is spoken into a ringing phone, and the person hears nothing or only the tail of the opening. Two settings control this, and both default to current behavior so existing agents are unchanged.

**Pause before speaking.** Set `firstMessageDelayMs` to wait that many milliseconds after the call is reported answered before the opening is spoken. Tune it to the route: start around 2000 to 3000 and adjust. If the person speaks during the pause, the opening is skipped and the agent simply replies to them, so the two never talk over each other.

```json
{ "firstMessageDelayMs": 2500 }
```

**Let the caller speak first.** Set `startSpeaker` to `"user"` and the agent stays silent until the person says something, then opens the conversation from its prompt. Nothing is spoken during ringing at all. The re-engagement prompt is held as well, so the silence is genuine rather than a short delay.

```json
{ "startSpeaker": "user" }
```

Use the pause when you want the agent to open the call but slightly later, and `startSpeaker` when you would rather the person open it. A third option needs no settings at all: keep the opening to a short line such as "Hi, is this Alex?" so that little is lost if part of it is missed.
</Note>

### Separate Inbound and Outbound Openings

Agents that handle both directions often need different openings: "Thank you for calling..." inbound versus "Hi, this is ... calling from ..." outbound. Set the **Outbound First Message** field (API: `firstMessageOutbound`) with the outbound text, and keep **First Message** as the inbound text. On outbound calls the agent speaks the outbound text; on inbound calls (or when the outbound field is empty) it speaks First Message, so existing agents are unaffected until you set it.

Both fields support `{{variables}}` with the same substitution as the prompt, so per-call personalization works in either direction. This applies to static first messages; in dynamic mode the opening is generated from the prompt, which can branch on the `{{direction}}` variable instead.
...
5. If leaving a message, configure the voicemail message:
   ```
   Hi {{first_name}}, this is Acme Corp calling to confirm your appointment. Please call us back at 1-800-555-0123 at your earliest convenience. Thank you.
   ```
   The voicemail message supports `{{variables}}`, just like your prompt and first message. Variables are filled in from the call's variables (for example, `first_name`). If a variable has no value, the placeholder is left as-is.
6. Click **Save**
...
Some voicemail systems answer without any greeting the agent can hear, so there is nothing for voicemail detection to key on. The platform closes these calls automatically: if nobody has spoken by the third re-engagement attempt (and the call is at least 20 seconds old), the agent says a short sign-off and ends the call with hangup cause `SILENCE_TIMEOUT`. This applies only while the other side has said nothing at all. Once a person has spoken, the agent stays patient through quiet stretches and keeps re-engaging normally.

## Call Pacing and Concurrency

### Understanding Concurrency Limits

RevRing enforces a concurrency limit on the maximum number of simultaneous calls per organization. You can view your current limit and usage on the [Billing page](https://dashboard.revring.ai/app/billing).

### Call Queuing

When you send an outbound call and all concurrent call slots are in use, RevRing automatically **queues** the call. Queued calls are dispatched in approximate order as slots become available.

While a call is waiting, its status is `QUEUED`. Once a slot opens and the call is dispatched, the status transitions to `INITIATED` and then `ONGOING` when the recipient answers.

<Note>
Queued calls have a maximum wait time of approximately 4 hours. If a slot does not open within that window, or if the call fails to dispatch for another reason, the call transitions to `FAILED` rather than remaining `QUEUED` indefinitely. Inspect the `errorMessage` field to see why.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Hit concurrency limit (429 error)">
    ...
    - Check your current limit and usage at [Billing](https://dashboard.revring.ai/app/billing)
