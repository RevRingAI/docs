...
| `{{language}}` | The agent's language code | `en` |
...
```
Hi {{customer_name}}, this is Alex from {{business_name}}. How can I help you today?
```
...
When the call begins, greet the caller:
- If {{direction}} is inbound, thank them for calling {{business_name}} and ask how you can help.
- If {{direction}} is outbound, introduce yourself and explain you are calling to confirm their appointment.
```
