...
<Note>
Custom tools also run in [web widget chat](/platform/web-widget). Chat invocations use the same request format, with `callId` set to `null` and two extra fields — `conversationId` and `channel` (`"web_chat"`) — identifying the chat thread instead of a call.
</Note>
