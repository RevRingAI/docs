...
| **Caller ID Shown to Recipient** | Which number the transfer recipient sees. `agent` (default) shows the agent's own number for the call. `original_caller` shows the customer's number, so the recipient sees who is on the line. |
| **No Answer Timeout** | How long the destination may ring before the attempt is abandoned. 10 to 300 seconds, default 75. |
| **Wait for a Person** | How long to wait after the destination answers for a person to actually come on the line. Covers queues and hold systems that pick up but never hand over. 15 to 600 seconds, default 120. |

<Note>
**Presenting the original caller's number**: a warm transfer places a new outbound call to the recipient over your own trunk, so `callerIdMode: "original_caller"` sends a caller ID your account does not own. Support for this is set by your telephony provider, not by RevRing. Many providers allow it, often as a per-trunk option named something like ANI override or caller ID passthrough, and some require it to be enabled before it takes effect. Others only permit caller IDs you own or have verified in advance. If the caller withheld their number, or it is not a usable phone number, the agent's number is presented instead. If your provider only permits owned or verified caller IDs, the provider refuses the dial and the transfer attempt fails, so confirm support with your provider before enabling this option on a live agent.

A cold transfer is handed off by your provider rather than re-dialed, so the caller ID it presents is governed by your trunk's own transfer settings.
</Note>

<Note>
**Reaching an extension**: If a route has an **Extension**, the agent dials it as touch tones the moment the recipient's line answers — sooner than any person could dial by hand. Many phone menus and auto-attendants are not ready for input in that first instant, and digits that arrive too early are silently ignored, leaving the call wherever the line answered (typically the main menu or front desk).

To avoid this, begin the extension with `w` pauses. Each `w` waits 0.5 seconds before the digits are dialed, and we recommend starting with `wwww` (2 seconds), which works with nearly all phone systems:
...
If the destination still doesn't pick up the digits, add more `w`s. Extensions are dialed on warm transfers only, because a cold transfer hands the call off immediately.
</Note>
...
curl -X PATCH https://api.revring.ai/v1/agents/{agentId} \
  ...
  -d '{
    ...
    "warmTransferConfig": {
      ...
      "dialTimeoutSeconds": 75,
      "waitForHumanSeconds": 120
    }
  }'
```

**If nobody answers**: the agent lets the destination ring for `dialTimeoutSeconds` seconds (10 to 300, default 75), then tells the caller it could not connect and continues the conversation. The caller can ask to try again, or the conversation can fall through to another route. Raise the timeout for teams that take longer to pick up; lower it to fail over to a backup route faster.

**If the line answers but nobody comes on**: some destinations pick up into a queue, a hold system, or an unattended handset. The agent waits `waitForHumanSeconds` (15 to 600, default 120) for a person to speak. If nobody does, it ends the attempt, releases that leg, tells the caller the destination answered but no one came on the line, and continues the conversation. Raise it for queues that routinely hold callers for several minutes.

Both timeouts can also be set per route, which is useful when one agent transfers to both a fast internal desk and a slow queue line:

```json
"transferRoutes": [
  { "to_number": "+12025550123", "condition": "Billing questions", "mode": "warm", "dialTimeoutSeconds": 30 },
  { "to_number": "+12025550199", "condition": "Claims queue", "mode": "warm", "dialTimeoutSeconds": 120, "waitForHumanSeconds": 300 }
]
```
...
### Ignore Brief Acknowledgments

Callers naturally say things like "okay", "yeah", or "uh-huh" while listening. By default, these brief acknowledgments do not interrupt the agent, so it finishes its sentence smoothly instead of stopping mid-thought. Anything beyond a brief acknowledgment, such as "wait", "stop", a question, or "okay but wait", interrupts the agent immediately. When the agent is not speaking, acknowledgment words behave like any other reply.

This is enabled by default with a recommended list that covers common acknowledgments and adapts to the agent's language setting (for example, Spanish agents also recognize "ajá" and "claro"). You can customize it per agent:

- **Custom word list**: replace the recommended list with your own words or short phrases, for example to cover another language.
- **Disable**: turn the toggle off (or set `interruptionIgnoreWords` to `[]` via API) so every caller utterance can interrupt.
...
curl -X PATCH https://api.revring.ai/v1/agents/{agentId} \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "interruptionIgnoreWords": ["okay", "yeah", "got it", "claro"]
  }'
```

### Interruption Word Count

Controls how many words a caller must say before the agent stops talking. The default of 1 keeps the agent responsive to any real interjection. Raise it to make the agent harder to interrupt, or set it to 0 to stop the agent on any caller sound, including background noise.

Urgent phrases such as "stop", "wait", and "hold on" always interrupt the agent immediately, regardless of this setting.
...
## Live Appointment Booking with Cal.com

RevRing hosts a Cal.com connector so agents can check real availability and book appointments live on a call, with no backend required. Two custom tools point at the hosted endpoints and carry your Cal.com API key in server side header configuration, where the model never sees it.

Register these two tools on the agent (dashboard Custom Tools, or `POST /v1/agents/{agentId}/custom-tools`), replacing the key, event type id, and timezone:

```json
{
  "name": "Check Availability",
  "webhookUrl": "https://api.revring.ai/public/integrations/calcom/check-availability?eventTypeId=YOUR_EVENT_TYPE_ID&timeZone=America/New_York",
  "enabled": true,
  "headers": [{ "key": "x-cal-api-key", "value": "YOUR_CAL_API_KEY" }],
  "schema": {
    "type": "function",
    "name": "check_availability",
    "description": "Check the calendar for open appointment times. Use when the caller asks about available times or wants to schedule.",
    "parameters": { "type": "object", "properties": {
      "date": { "type": "string", "description": "Date to check availability (YYYY-MM-DD). Defaults to today." },
      "days_ahead": { "type": "number", "description": "Days ahead to check. Default 2. Max 5." } } }
  }
}
```
...
```json
{
  ...
  "webhookUrl": "https://api.revring.ai/public/integrations/calcom/book-appointment?eventTypeId=YOUR_EVENT_TYPE_ID&timeZone=America/New_York",
  "enabled": true,
  "headers": [{ "key": "x-cal-api-key", "value": "YOUR_CAL_API_KEY" }],
  "schema": {
    "type": "function",
    "name": "book_appointment",
    "description": "Book an appointment on the calendar. Only use after the caller has confirmed a specific date and time.",
    "parameters": { "type": "object", "required": ["date", "time"], "properties": {
...
Then add booking behavior to the prompt: check availability before offering times, offer only times the tool returned, collect the email (and phone when the calendar requires it), book only after the caller confirms, and confirm only after the tool reports success. Have the agent speak a short bridge line ("Let me check what is open...") before each tool call so the lookup never sounds like dead air.

The tools return short spoken ready text, including clear guidance on failures (slot taken, phone number required), so the agent recovers naturally. Phone numbers are normalized automatically, and both Cal.com response formats are handled. If you prefer to run the endpoints on your own backend for custom logic, point the same tools at your URLs instead and keep the key server side.
