...
| **Summary Prompt** | Custom prompt used to generate the brief for the recipient. Max 2,000 characters. If omitted, a default summary is generated. |
| **Intro Message** | Message spoken to the recipient before connecting (e.g., "I'm transferring a customer who needs help with a refund.") Max 500 characters. |

#### Configuring via API

Set `transferMode` and `warmTransferConfig` when creating or updating an agent:
    ...
    "transferRoutes": [
      {
        ...
        "condition": "Transfer to support team for technical issues"
      }
    ],
    "warmTransferConfig": {
      "holdMessage": "Please hold while I connect you with our support team.",
      "holdMusicEnabled": true,
      "summaryPrompt": "Summarize the customer issue in 1-2 sentences for the support agent.",
      "introMessage": "I have a customer on the line who needs technical assistance."
    }
  }'
```
...
The Keypad Input tool allows the agent to send DTMF touch tones during a call. This is useful when navigating IVR (interactive voice response) systems — for example, when the agent needs to press digits to navigate a phone menu during a transferred call or an outbound call to a phone system.
...
1. Navigate to the **Tools** tab
2. Locate **Keypad Input (DTMF)** under System Tools
3. Toggle the switch to enable
4. Optionally set a **Description Override** to customize when the agent uses this tool
5. Click **Save**
...
By default, callers can interrupt the agent while it's speaking. You can disable this behavior so the agent always finishes speaking before listening for input.
...
2. Toggle **Disable Interruptions** to on
3. Click **Save**
...
<Note>
Disable interruptions applies to all agent speech globally. In [Conversation Flows](/platform/conversation-flows), you can set `blockInterruptions` on individual nodes for more granular control.
</Note>
