...
| **Wait for a Person** | How long to wait after the destination answers for a person to actually come on the line. Covers queues and hold systems that pick up but never hand over. 15 to 600 seconds, default 300. |
...
**If the line answers but nobody comes on**: some destinations pick up into a queue, a hold system, or an unattended handset. The agent waits `waitForHumanSeconds` (15 to 600, default 300) for a person to speak. If nobody does, it ends the attempt, releases that leg, tells the caller the destination answered but no one came on the line, and continues the conversation. Raise it for queues that routinely hold callers for several minutes.
