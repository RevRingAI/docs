...
When enabled, the agent can invoke the `transfer_to_number` function with the destination number. The call will be transferred, and the agent's involvement ends. The transfer appears in the call transcript showing:
